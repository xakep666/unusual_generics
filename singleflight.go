...
type call[T any] struct {
	...
	dups  int
	chans []chan<- SingleFlightResult[T]
}

// SingleFlightGroup represents a class of work and forms a namespace in
// which units of work can be executed with duplicate suppression.
// Based on https://pkg.go.dev/golang.org/x/sync/singleflight#Group.
...
func (g *SingleFlightGroup[T]) DoChan(key string, fn func() (T, error)) <-chan SingleFlightResult[T] {
	...
	c := &call[T]{chans: []chan<- SingleFlightResult[T]{ch}}
	c.wg.Add(1)
	g.m[key] = c
	g.mu.Unlock()
...
func (g *SingleFlightGroup[T]) doCall(c *call[T], key string, fn func() (T, error)) {
	normalReturn := false
	recovered := false
	...
	defer func() {
		...
		if e, ok := c.err.(*panicError); ok {
			...
			for _, ch := range c.chans {
				ch <- SingleFlightResult[T]{c.val, c.err, c.dups > 0}
			}
		}
	}()
